...
def validate_bundle(b):
    try:
        v=b['visibility']; p=b['profile']; r=b['request']; h=b['health']; u=b['update']; g=b['registry']; m=b['material']
    ...
    if not all(p.get(k) for k in ('profile_id','service_name','work_dir','repository_secret_scope')): return block('BLOCKED_RUNNER_PROFILE_IDENTITY_INCOMPLETE')
    labels=set(p.get('labels') or [])
    if not {'self-hosted','macOS','ARM64'}.issubset(labels): return block('BLOCKED_RUNNER_PROFILE_LABELS')
    ...
    if u.get('profile_id')!=p.get('profile_id') or u.get('runner_version')!=h.get('runner_version') or u.get('update_state') not in ('CURRENT','UPDATED'): return block('BLOCKED_RUNNER_UPDATE_STATE')
    rec=b.get('receipt')
    if rec is not None:
        if rec.get('request_id')!=r.get('request_id') or rec.get('profile_id')!=p.get('profile_id') or rec.get('repository_full_name')!=repo or rec.get('source_sha')!=r.get('source_sha') or rec.get('source_tree')!=r.get('source_tree'): return block('BLOCKED_RUNNER_RECEIPT_IDENTITY')
...
def self_test(root):
    ...
    for case in cases:
        sample=copy.deepcopy(positive); set_path(sample,case['path'],case['value'])
        actual=validate_bundle(sample)
        if actual!=case['expected_blocker']:
            raise SystemExit(f"{case['name']}: expected {case['expected_blocker']} got {actual}")
